function appendTextToImgSVG(imgSelector, svgText) {
  ...
  fetch(imgElement.src)
      ...
      .then(svgContent => {
          ...
          textElement.setAttribute("y", minY + height / 2);
  ...
  selectableDivs.forEach(div => {
      ...
      div.addEventListener("click", () => {
          ...
          if (imgElement){
            ...
            switch (currentDie) {
              case "d6-0":
                  newImg.src = "images/dice/d6/d6-3.svg";
                  newImg.id = "d6-3";
                  break;
              case "d6-3":
                  newImg.src = "images/dice/d6/d6-6.svg";
                  newImg.id = "d6-6";
                  break;
              case "d6-6":
                newImg.src = "images/dice/d6/d6-0.svg";
                newImg.id = "d6-0";
                  break;
            }
...
function createDice(diceType) {
  ...
  switch (diceType) {
    ...
    case '6':
      ...
      diceImg.src = `images/dice/d6/d6-${randomNumber}.svg`; // Use the random number in the file path
      diceImg.id = `d6-${randomNumber}`;
      break;
    case '20':
  ...
  setTimeout(() => diceImg.classList.add("show"), 10);
...
const diceSelector = document.querySelector(".selectableDieImage");
const regex = /(?<=d)(\d+)(?=-)/;
diceSelector.addEventListener("click", function(event){
